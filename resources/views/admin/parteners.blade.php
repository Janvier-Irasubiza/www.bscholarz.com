...
<x-app-layout>
    ...
    <x-slot name="header"> </slot>

        <div style="padding: 0px 20px 32px 20px"> <div
            class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-2 mt-2">
...
<div class="d-flex gap-5 align-items-center">
                    <div>
                    <img class="saying-pp-cst" src="{{ asset('profile_pictures') }}/{{ $partener -> profile_picture }}" id="output" alt="Profile">
                </div>

                <div class="w-full">
                <h5 class="modal-title" id="staticBackdropLabel">

                    <div class="widget-subheading"><p class="mb-0" style="font-size: 25px"> {{ $partener -> names }} </p> </div>
    ...
    <div class="card-footer mt-3 py-3 pb-0 flex-section justify-content-between align-items-center" style="border-top: 1px solid #00000023">
    ...
    <div class="btn-actions-pane-right d-flex flex-row-reverse align-items-center text-capitalize">
        ...
        <div class="widget-numbers text-success"><span style="font-weight: 600; font-size: 30px"> {{ number_format(round($partener -> pending_amount)) }} </span></div>

        <button class="assistantInfo" id="disburse" data-id="{{ $partener ->  id }}" data-names="{{ $partener ->   names }}" data-email="{{ $partener ->   email }}" data-phone="{{ $partener ->  phone_number }}" data-amount="{{ $partener -> pending_amount }}" data-toggle="modal" data-target="#invalidate">
    ...
    @endforeach
          ...
          <!-- Modal -->
          <div class="modal fade" id="invalidate" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    <div class="modal-dialog w-mdal modal-dialog-centered d-flex align-items-center" role="document">
                        <div class="modal-content">
                        <div class="modal-header">
                            <div>
                                <h5 class="phone-number contact" style="text-align: left; font-size: 25px; font-weight: 600" id="staticBackdropLabel"></h5>
                                <small style="font-size: 17px;" class="mb-0"><span class="modal-title names"> </span> <br> <span style="font-size: 14px" class="text-muted email"></span></small>
                            </div>                            
                      ...
                      <input style="padding: 6px 10px; text-align: left; border: 1px solid #00000050; color: #808080; border-radius: 4px" name="partner" id="assistant" class="block mt-1 w-full input-holder assistant" hidden/>
                      <input style="padding: 6px 10px; text-align: left; border: 1px solid #00000050; color: #808080; border-radius: 4px" name="full_amount" id="full_amount" class="block mt-1 w-full input-holder assistant" hidden/>
                      <div class="widget-numbers text-success mt-0 mb-0"><span style="font-weight: 600; font-size: 30px" class="amount"></span></div>
                          <button type="submit" id="send_btn" style="padding: 5px 30px; font-weight: 600; color: ghostwhite; text-transform: uppercase; font-weight: bold; font-size: 10px; " class="btn apply-btn mt-2">Disburse</button>
                        </form>
                          </div>
                      ...
                      <input style="padding: 6px 10px; text-align: left; border: 1px solid #00000050; color: #808080; border-radius: 4px" name="partner" id="assistant" class="block mt-1 w-full input-holder assistant" hidden/>
                            <input placeholder="Enter amount to disburse here..." id="reply_input" name="partial_amount" class="block mt-1 w-full" style="border: 2px solid #000; border-radius: 6px; padding: 6px; font-size: 14px" required />
                            <div class="mt-3 mb-2 text-right d-flex justify-content-end">
                            <button type="submit" id="send_btn" style="padding: 5px 30px; font-size: 10px; font-weight: 600; color: ghostwhite; text-transform: uppercase; font-weight: bold" class="btn apply-btn">Disburse</button>
                        </div>
                        </form>
                      </div>
