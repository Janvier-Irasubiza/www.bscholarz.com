...
<div style="background: none;" class="w-full sm:max-w-md mt-0 px-6 py-4 pt-0 overflow-hidden sm:rounded-lg">
        ...
        <div class="d-flex justify-content-center align-items-center mb-5 py-3" style="border-bottom: 1px solid rgba(0, 0, 0, 0.151);">
                ...
                <h1 style="font-size: 30px" class="ml-3 mb-2">Partner - Login</h1>

        </div>
    <!-- Session Status -->
    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('rhythmbox.auth') }}">
        @csrf
      ...
      @endif
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('partner-password.request'))
                <a class="underline text-sm hover:text-black-900 dark:hover:text-black-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('partner-password.request') }}">
