    ...
    <form method="POST" action="{{ route('staff-password.store') }}">
      ...
      @endif
      	...
      	@if(!empty($error)) 
      		...
      		{{ $request->email }} <br> <br> {{ $error }}, 
      			<a class="underline text-sm hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('staff-password.request') }}">
      ...
      <!-- Password Reset Token -->
        ...
        <div>
            ...
            <x-text-input id="email" readonly class="block mt-1 w-full" type="email" name="email" :value="old('email', $request->email)" required autofocus autocomplete="username" disabled/>
