...
<div style="padding: 20px 32px;">
    ...
    <form method="POST" action="{{ route('add-client-app') }}" class="mt-6 space-y-6 mt-4 mb-3">
        ...
        <div class="alert alert-secondary mb-3" role="alert">
            ...
            <small> Is it another application for <strong>{{ Session::get('email_error') }}</strong>? If no, use different email address. </small>

            <input type="text" name="applicant_id" value="{{  Session::get('applicant') }}" hidden>

            <button class="apply-btn" style="color: ghostwhite; font-size: 14px" >Yes it's another application</button>
    ...
    <form method="POST" action="{{ route('record-new-activity') }}" class="mt-6 space-y-6 mt-4 mb-3">
        @csrf
    ...
    @endif
        ...
        <div>
            ...
            <select id="activityName" name="activityName" type="text" class="mt-1 block w-full" required autocomplete="activityName" value="{{ old('activityName') }}" style="border: 2px solid; border-radius: 7px; padding: 5px 8px; font-size: 14px">
                ...
                <option value="{{ $discipline -> id }}" selected>{{ $discipline -> discipline_name }}</option>
            ...
            @if(old('customActivityName'))
            <x-text-input style="display: block" id="txt-custom" name="customActivityName" type="text" class="mt-1 block w-full" value="{{ old('customActivityName') }}" autocomplete="customActivityName" placeholder="Enter activity name"/>
            @else 
            <x-text-input id="txt-custom" name="customActivityName" type="text" class="mt-1 block w-full" value="{{ old('customActivityName') }}" autocomplete="customActivityName" placeholder="Enter activity name"/>
            @endif
        ...
        <div>
            ...
            <x-text-input id="consumerNames" name="consumerNames" type="text" class="mt-1 block w-full" value="{{ old('consumerNames') }}" required autocomplete="consumerNames" />
            <x-input-error class="mt-2" :messages="$errors->get('consumerNames')" />
        </div>

        <div>
            <x-input-label for="consumerEmail" :value="__('Consumer Email')" />
            <x-text-input id="consumerEmail" name="consumerEmail" type="text" class="mt-1 block w-full" value="{{ old('consumerEmail') }}" required autocomplete="consumerEmail" />
        ...
        <div>
            ...
            <x-text-input id="consumerPhoneNumber" name="consumerPhoneNumber" type="text" class="mt-1 block w-full" value="{{ old('consumerPhoneNumber') }}" required autocomplete="consumerPhoneNumber" />
            <x-input-error class="mt-2" :messages="$errors->get('consumerPhoneNumber')" />
        </div>

        <div id="customActivityCost">
            <x-input-label for="serviceCost" :value="__('Service Cost')" />
            <x-text-input id="serviceCost" name="serviceCost" type="text" class="mt-1 block w-full" value="{{ old('serviceCost') }}" autocomplete="serviceCost" />
            <x-input-error class="mt-2" :messages="$errors->get('serviceCost')" />
        </div>

        <div>
            <x-input-label for="paymentStatus" :value="__('Payment Status')" />
            <select id="paymentStatus" name="paymentStatus" type="text" class="mt-1 block w-full" value="{{ old('paymentStatus') }}" required autocomplete="paymentStatus" style="border: 2px solid; border-radius: 7px; padding: 5px 8px; font-size: 14px">
        ...
        <div id="amountPaid" style="display: block">
            ...
            <x-text-input id="receivedAmount" name="receivedAmount" type="text" class="mt-1 block w-full" value="{{ old('receivedAmount') }}" autocomplete="receivedAmount" placeholder="What amount have you received?"/>
        ...
        <div>
            ...
            <textarea id="desc" name="desc" class="mt-1 block w-full" style="border: 2px solid #000; border-radius: 6px" required autocomplete="desc"> {{ old('desc') }} </textarea>
