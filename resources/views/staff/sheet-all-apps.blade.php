...
<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-2 mt-2 mb-4">
                    ...
                    <p class="text-muted mb-0" style="font-size: 13px">Served requests by {{ Auth::guard('staff') -> user() -> names }} </p>
                    </div>

                    <div class="col-lg-3 d-flex gap-2">
                        <a href="{{ route('sort-recs-this-week', ['assistant' => Auth::guard('staff') -> user() -> id]) }}">
                            <p class="fw-normal mb-1">
                                <span class="badge rounded-pill px-3" style="color: #000;">This week</span>
                            </p>
                        </a>
                    ...
                    <table id="table" class="table align-middle mb-0 bg-white">
                        ...
                        @foreach ($completedApp as $application) 

                        @if($application -> remittance_status == 'Paid')    
                            ...
                            @endif
                               ...
                               @php
                                  ...
                                  $atdg_amt_exp = explode(';', $application -> outstanding_paid_amount);      
                            ...
                            <td class="text-left">
                            <p class="fw-normal mb-1">Paid: <strong>{{ number_format($sum + $application -> amount_paid) }}</strong></p>
                            <p class="fw-normal mb-1"> O/S: <strong>{{ number_format($application -> outstanding_amount - $sum) }}</strong></p>
                            </td>
                            ...
                            @else
                            <p class="fw-normal mb-1">
                            <span class="badge bg-secondary rounded-pill px-3">{{ $application -> remittance_status }}</span>
                            </p>
                            @endif
