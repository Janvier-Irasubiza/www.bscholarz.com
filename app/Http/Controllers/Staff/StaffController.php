            ...
            if (empty($request -> customActivityName)) {
                ...
                if ($request -> paymentStatus == 'Paid') {
                  ...
                  $fee = DB::table('disciplines') -> where('id', $request -> activityName) -> select('service_fee') -> first();
                    ...
                    foreach ($partners as $partner) {
                        ...
                        $percentage = $partner -> pending_amount + (($fee -> service_fee * $partner -> percentage) / 100);
                    ...
                    $commision = ($fee -> service_fee * Auth::guard('staff') -> user() -> percentage)/100;
        
                    array_push($activity_application_info, [
                        'applicant' => $userId -> id,
                        'discipline_id' => $request -> activityName,
                        'payment_status' => $request -> paymentStatus,
                        'amount_paid' => $fee -> service_fee,
                        'payment_date' => date('Y-m-d H:i:s'),
                        'status' => 'Complete',
                        'assistant' => Auth::guard('staff') -> user() -> id,
                        'application_type' => 'Custom',
                ...
                elseif ($request -> paymentStatus == 'Partial-payment') {
                    ...
                    array_push($activity_application_info, [
                        ...
                        'payment_status' => 'Paid',
                        'amount_paid' => $request -> receivedAmount,
                        'outstanding_amount' => $app_req -> service_fee - $request -> receivedAmount,
                        'payment_date' => date('Y-m-d H:i:s'),
                        'status' => 'Complete',
                        'assistant' => Auth::guard('staff') -> user() -> id,
                        'application_type' => 'Custom',
                    ...
                    array_push($activity_application_info , [
                        ...
                        'discipline_id' => $request -> activityName,
                        'payment_status' => $request -> paymentStatus,
                        'outstanding_amount' => $app_req -> service_fee,
                        'payment_date' => date('Y-m-d H:i:s'),
                        'status' => 'Complete',
                        'assistant' => Auth::guard('staff') -> user() -> id,
                        'application_type' => 'Custom',
